...
class BbgRatesUpdatePrices():
    ...
    def update(self, ticker_list=[]):
        ...
        df = execute_postgresql_query(f"""
            ...
            FROM public.{settings.BBG_RATES_TABLE}
            GROUP BY ticker;
        """)
        for start_date, ticker_list in df.groupby('max_date')['ticker'].apply(list).items():
            ...
            if not px_last_df.empty: 
                ...
                for index in px_last_wide_df:
                    ...
                    upload_dataframe_to_postgresql(index_df, table_name=settings.BBG_RATES_TABLE)
        return True


if __name__ == '__main__':
    print(f"[START] {datetime.now()}")
    bbg_update_prices = BbgRatesUpdatePrices().update()
